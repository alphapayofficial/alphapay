Alphapay Core version 0.13.0 is now available from:
...
In addition to that, with 0.12.x there have been varied reports of Alphapay Core
...
Various code modernizations have been done. The Alphapay Core code base has
...
HD wallets are incompatible with older versions of Alphapay Core.
...
proposed parameters, a future release of Alphapay Core is expected that
...
The two phases are distinct in the Alphapay-Qt GUI. During the first one,
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use APY/kB (MarcoFalke)
